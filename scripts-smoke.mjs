...
try{
  ...
  assert.ok(videoApi.includes('Do not generate spoken dialogue or narration'),'Separate approved voice/video audio strategy missing');
  ...
  assert.ok(appSource.includes('Generate video clip')&&appSource.includes('Submitting…')&&appSource.includes('rendering in the background'),'Responsive scene video-generation UX missing');
  ...
  assert.ok(appSource.includes('videoPollers')&&appSource.includes('resumePendingVideoPolls')&&appSource.includes('attempt<6?5000:8000'),'Background/adaptive video polling missing');
  ...
  assert.ok(appSource.includes('signInWithGoogle')&&appSource.includes('/auth/v1/authorize?provider=google'),'Google OAuth sign-in entry point missing');
  ...
  assert.ok(appSource.includes('sceneFinalVoiceUrls')&&appSource.includes("cachedAudioRequest('/api/tts'"),'Approved ElevenLabs voice mix is not included in final rendering');
  assert.ok(appSource.includes('createMediaStreamDestination')&&appSource.includes('ambientGain.gain.value=.34'),'Final audio mixing graph missing');
  ...
  assert.ok(videoApi.includes('function framingDirection')&&videoApi.includes('Keep important faces fully inside the frame'),'Video safe-framing prompt missing');
...
console.log('CineTale v1.9.3 smoke tests passed: auth, workspace/library, responsive scene controls, quota-aware one-click final production, Veo Lite fallback, safe paced generation, transient provider retry, preloaded final render, approved ElevenLabs audio, final video render/download/share, final assembly, voice filtering, navigation and DOM integrity.');
} finally {
