...
export function demoProject(input={}){
  ...
  return {
    id:`ct_${Date.now()}`,
    title,
    logline:format==='Episode'?`${idea} What begins as a discovery becomes an ongoing story about trust, consequence, and a secret that keeps changing what the characters think they know.`:`${idea} What begins as a discovery becomes a complete ${format.toLowerCase()} about trust, consequence, and the choice the characters make by the end.`,
    ...
    worldBible:{
      ...
      rules:format==='Episode'?['Character identity remains consistent unless explicitly changed by the creator.','Important story facts become canon and carry into later episodes.','Every episode earns its cliffhanger rather than ending arbitrarily.']:['Character identity remains consistent unless explicitly changed by the creator.','Important story facts stay consistent from beginning to ending.','The standalone ending resolves the central dramatic question rather than forcing a sequel.'],
