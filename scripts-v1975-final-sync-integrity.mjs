...
must(app.includes("const APP_VERSION = '1.9.81'"),'app version must be 1.9.81');
...
must(app.includes('const sceneDuration=Math.max(primaryDuration,Math.min(Math.max(primaryDuration,requested),visualDuration));'),'scene duration must be bounded by unique real visual coverage');
must(app.includes('useEmbeddedSyncedAudio:spoken&&synced'),'speaking scenes must use synchronized media audio');
must(app.includes('const allowAudio=Boolean(useEmbeddedSyncedAudio&&item.entry?.synchronized)')&&app.includes('video.muted=!allowAudio'),'only validated synchronized timeline media may contribute audio');
...
console.log('v1.9.81 final sync/no-repeat/auth-branding regression: PASS');
