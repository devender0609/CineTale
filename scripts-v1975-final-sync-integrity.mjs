...
must(app.includes("const APP_VERSION = '1.9.75'"),'app version must be 1.9.75');
must(app.includes("pipelineVersion:9"),'final pipeline version must be 9');
must(app.includes("durationMode:'source-coverage-no-repeat-synced-audio'"),'new deterministic duration mode missing');
...
must(app.includes('const sceneDuration=Math.max(primaryDuration,Math.min(Math.max(primaryDuration,requested),visualDuration));'),'scene duration must be bounded by real visual coverage and synchronized primary duration');
...
console.log('v1.9.75 final sync/no-repeat/auth-branding regression: PASS');
