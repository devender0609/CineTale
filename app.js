...
async function pollVideo(i,operation,button,{background=false}={}){
  const p=current(),ep=episodeOf(p);if(!p||!ep)return;
  const key=videoPollKey(p.id,ep.id||ep.number,i);
  if(videoPollers.has(key))return videoPollers.get(key);
  const job=(async()=>{
    for(let attempt=0;attempt<72;attempt++){
      const live=current(),liveEp=episodeOf(live),scene=liveEp?.scenes?.[i];
      ...
      // Check quickly at first, then back off. This improves perceived responsiveness without hammering the API.
      ...
      if(d.status==='ready'){
        ...
        updateProject(x=>{const e=episodeOf(x);if(!e?.scenes?.[i])return;const target=e.scenes[i];target.videoUrl=d.videoUrl;target.videoOperation=null;target.videoQueuedAt=null;target.videoError=null;target.videoPlaybackError=null;resetSceneLipSyncForNewSource(target,d.videoUrl);x.videoStatus='ready';x.finalAssembly=null;x.renderStatus=null;x.finalVideoMeta=null});
        toast('Scene video clip is ready.');scheduleSceneLipSyncAfterSourceReady(p.id,ep.id||ep.number,i);return;
      }
      if(d.status==='error'){
        confirmedVideoOperations.delete(operation);
        updateProject(x=>{const e=episodeOf(x);if(e?.scenes?.[i]){e.scenes[i].videoOperation=null;e.scenes[i].videoQueuedAt=null;e.scenes[i].videoError=d.error||'Video generation failed'}});
        throw new Error(d.error||'Video generation failed');
      }
    }
    confirmedVideoOperations.delete(operation);
    updateProject(x=>{const e=episodeOf(x),target=e?.scenes?.[i];if(target&&target.videoOperation===operation){target.videoOperation=null;target.videoQueuedAt=null;target.videoError='Video rendering timed out. Your previous clip was preserved; try Regenerate clip again.'}});throw new Error('Video rendering timed out. Your previous clip was preserved; try Regenerate clip again.');
