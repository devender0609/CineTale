...
assert.ok(app.includes('p.requestedFormat=p.format')&&app.includes('normalizedFormat'),'Format persistence invariant missing');
assert.ok(app.includes('scale=Math.min(width/vw,height/vh)'),'Final-video renderer still crops source frames');
assert.ok(!css.includes('object-fit:cover'),'Generated-media CSS still contains cover cropping');
assert.ok(app.includes('story beat')&&app.includes('target ${cfg.finalName} runtime'),'Runtime semantics are not creator-facing/explicit');
assert.ok(app.includes('function ensureCharacterIdentityIds')&&app.includes('function resolveDialogueCharacterIndex'),'Stable character identity resolver missing');
assert.ok(app.includes('dialogueBindings')&&app.includes('characterId'),'Scene dialogue is not bound to persistent character IDs');
assert.ok(app.includes('bindSceneDialogueCharacters(x,target,{preserveExisting:true})'),'Scene performance editor does not preserve character identity bindings');
